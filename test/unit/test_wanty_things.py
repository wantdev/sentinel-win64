...
def test_valid_want_address():
    from wantlib import is_valid_want_address

    main = valid_want_address()
    test = valid_want_address('testnet')

    assert is_valid_want_address(main) is True
    assert is_valid_want_address(main, 'mainnet') is True
    assert is_valid_want_address(main, 'testnet') is False

    assert is_valid_want_address(test) is False
    assert is_valid_want_address(test, 'mainnet') is False
    assert is_valid_want_address(test, 'testnet') is True


def test_invalid_want_address():
    from wantlib import is_valid_want_address

    main = invalid_want_address()
    test = invalid_want_address('testnet')

    assert is_valid_want_address(main) is False
    assert is_valid_want_address(main, 'mainnet') is False
    assert is_valid_want_address(main, 'testnet') is False

    assert is_valid_want_address(test) is False
    assert is_valid_want_address(test, 'mainnet') is False
    assert is_valid_want_address(test, 'testnet') is False
...
def test_hash_function():
    ...
    hex_hash = "%x" % wantlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(wantlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(wantlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(wantlib.blocks_to_seconds(16616)) == 2612035
