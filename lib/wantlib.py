...
def is_valid_want_address(address, network='mainnet'):
    ...
    want_version = 140 if network == 'testnet' else 63
    ...
    try:
        ...
        # rescue from exception, not a valid Want address
        return False

    if (address_version != want_version):
        return False
...
# shims 'til we can fix the wantd side
def SHIM_serialise_for_wantd(sentinel_hex):
    ...
    # superblock => "trigger" in wantd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # wantd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    wantd_hex = serialise(obj)
    return wantd_hex


# shims 'til we can fix the wantd side
def SHIM_deserialise_from_wantd(wantd_hex):
    from models import SUCRD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(wantd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('want.conf')
